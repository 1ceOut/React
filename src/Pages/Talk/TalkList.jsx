...
const Talk = () => {
  ...
  const { userId, isLogin, LoginSuccessStatus, foodExpirationNotifications } =
    useUserStore();
  ...
  const fetchLastMessage = async (refrigerator_id) => {
    try {
      ...
      const response = await axios.get(
        `https://api.icebuckwheat.kro.kr/api/chatroom/${refrigerator_id}/last-message`
      );
      return response.data;
    } catch (error) {
      console.error(
        `Error fetching last message for fridge ${refrigerator_id}:`,
        error
      );
      return null;
    }
  ...
  const fetchUsersForFridge = async (refrigerator_id) => {
    try {
      const response = await axios.get(
        `https://api.icebuckwheat.kro.kr/api/food/find/refriUser?refrigerator_id=${refrigerator_id}`
      );
      ...
      console.error(
        `Error fetching users for fridge ${refrigerator_id}:`,
        error
      );
      return [];
    }
    ...
    const fetchMasterFridges = async () => {
      try {
        ...
        if (Array.isArray(masterFridges)) {
          ...
          //userId와 id를 문자열로 비교
          const filtered = masterFridges.filter(
            (fridge) => fridge.id == userId
          );
  ...
  const handleClick = (refrigeratorName, refrigerator_id) => {
    // 해당 냉장고의 userId만 넘김
    const selectedFridge = filteredFridges.find(
      (fridge) => fridge.refrigerator_id === refrigerator_id
    );

    if (selectedFridge) {
      navigate(
        `/Talk/TalkDetail?name=${refrigeratorName}&refri_id=${refrigerator_id}&id=${selectedFridge.id}`
      );
    } else {
      navigate(
        `/Talk/TalkDetail?name=${refrigeratorName}&refri_id=${refrigerator_id}&id=null`
      );
  ...
  return (
    <main className="flex flex-col items-center px-6 pt-5 pb-2 mx-auto w-full max-w-[390px] h-screen">
      ...
      <div>
        {data?.map((refri) => (
          <div
            key={refri.refrigerator_id}
            className="mt-8 w-[342px] h-10 flex items-center px-1 cursor-pointer"
            onClick={() =>
              handleClick(refri.refrigeratorName, refri.refrigerator_id)
            }
          >
